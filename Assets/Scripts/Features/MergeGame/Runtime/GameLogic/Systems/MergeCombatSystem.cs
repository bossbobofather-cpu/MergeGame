using System;
using System.Collections.Generic;
using MyProject.MergeGame.Events;
using MyProject.MergeGame.Models;
using Noname.GameAbilitySystem;

namespace MyProject.MergeGame.Systems
{
    /// <summary>
    /// 타워 공격 및 투사체 처리를 담당하는 전투 시스템입니다.
    /// 공격 판정은 Host에서만 수행합니다.
    /// </summary>
    public sealed class MergeCombatSystem
    {
        private sealed class PendingProjectile
        {
            /// <summary>
            /// Uid 필드입니다.
            /// </summary>
            public long Uid;
            /// <summary>
            /// PlayerIndex 필드입니다.
            /// </summary>
            public int PlayerIndex;
            /// <summary>
            /// AttackerUid 필드입니다.
            /// </summary>
            public long AttackerUid;
            /// <summary>
            /// TargetUid 필드입니다.
            /// </summary>
            public long TargetUid;
            /// <summary>
            /// SourceAscSnapshot 필드입니다.
            /// </summary>
            public AbilitySystemComponent SourceAscSnapshot;
            /// <summary>
            /// TargetingStrategy 필드입니다.
            /// </summary>
            public ITargetingStrategy TargetingStrategy;
            /// <summary>
            /// HitEffects 필드입니다.
            /// </summary>
            public List<GameplayEffect> HitEffects;
            /// <summary>
            /// Start 필드입니다.
            /// </summary>
            public Point3D Start;
            /// <summary>
            /// Impact 필드입니다.
            /// </summary>
            public Point3D Impact;
            /// <summary>
            /// RemainingTime 필드입니다.
            /// </summary>
            public float RemainingTime;
            /// <summary>
            /// TravelTime 필드입니다.
            /// </summary>
            public float TravelTime;
            /// <summary>
            /// ProjectileType 필드입니다.
            /// </summary>
            public ProjectileType ProjectileType;
            /// <summary>
            /// ThrowRadius 필드입니다.
            /// </summary>
            public float ThrowRadius;
            /// <summary>
            /// IsLanded 필드입니다.
            /// </summary>
            public bool IsLanded;
        }

        /// <summary>
        /// 활성 투사체 정보입니다 (스냅샷용).
        /// </summary>
        public readonly struct ActiveProjectile
        {
            /// <summary>
            /// Uid 속성입니다.
            /// </summary>
            public long Uid { get; }
            /// <summary>
            /// PlayerIndex 속성입니다.
            /// </summary>
            public int PlayerIndex { get; }
            /// <summary>
            /// Start 속성입니다.
            /// </summary>
            public Point3D Start { get; }
            /// <summary>
            /// Impact 속성입니다.
            /// </summary>
            public Point3D Impact { get; }
            /// <summary>
            /// Progress 속성입니다.
            /// </summary>
            public float Progress { get; }
            /// <summary>
            /// ProjectileType 속성입니다.
            /// </summary>
            public ProjectileType ProjectileType { get; }
            /// <summary>
            /// IsLanded 속성입니다.
            /// </summary>
            public bool IsLanded { get; }

            /// <summary>
            /// ActiveProjectile 생성자입니다.
            /// </summary>
            public ActiveProjectile(long uid, int playerIndex, Point3D start, Point3D impact, float progress, ProjectileType projectileType, bool isLanded)
            {
                Uid = uid;
                PlayerIndex = playerIndex;
                Start = start;
                Impact = impact;
                Progress = progress;
                ProjectileType = projectileType;
                IsLanded = isLanded;
            }
        }

        private readonly MergeHostState _state;
        private readonly TargetContext _targetContext;
        private readonly List<AbilitySystemComponent> _monsterAscList = new();
        private readonly List<PendingProjectile> _projectiles = new();
        private readonly List<PendingProjectile> _projectileRemoveBuffer = new();
        private readonly List<ActiveProjectile> _activeProjectileBuffer = new();
        private long _nextProjectileUid = 1;

        /// <summary>
        /// TargetContext 속성입니다.
        /// </summary>
        public TargetContext TargetContext => _targetContext;

        /// <summary>
        /// 특정 플레이어의 활성 투사체 목록을 반환합니다 (스냅샷용).
        /// </summary>
        public IReadOnlyList<ActiveProjectile> GetActiveProjectiles(int playerIndex)
        {
            _activeProjectileBuffer.Clear();
            for (var i = 0; i < _projectiles.Count; i++)
            {
                var p = _projectiles[i];
                if (p.PlayerIndex != playerIndex) continue;

                var progress = p.TravelTime > 0f ? 1f - (p.RemainingTime / p.TravelTime) : 1f;
                if (p.IsLanded) progress = 1f;

                _activeProjectileBuffer.Add(new ActiveProjectile(
                    p.Uid, p.PlayerIndex, p.Start, p.Impact,
                    progress, p.ProjectileType, p.IsLanded));
            }
            return _activeProjectileBuffer;
        }
        /// <summary>
        /// MergeCombatSystem 메서드입니다.
        /// </summary>

        public MergeCombatSystem(MergeHostState state)
        {
            _state = state ?? throw new ArgumentNullException(nameof(state));
            _targetContext = new TargetContext(GetEnemiesForTower, GetAlliesForTower, GetPositionForAsc);
        }

        /// <summary>
        /// 타워의 능력 활성화 결과(TargetData)를 바탕으로 공격을 실행합니다.
        /// </summary>
        public void ExecuteTowerAttack(
            long tick,
            int playerIndex,
            MergeTower tower,
            GameplayAbility ability,
            TargetData targetData,
            List<MergeGameEvent> events)
        {
            if (tower == null || events == null)
            {
                return;
            }

            // Throw 타입은 타겟 없이도 공격 가능 (경로 랜덤 위치).
            if (targetData == null && tower.ProjectileType != ProjectileType.Throw)
            {
                return;
            }

            if (tower.AttackType == TowerAttackType.HitScan)
            {
                ProcessHitScanAttack(tick, playerIndex, tower, ability, targetData, events);
                return;
            }

            ProcessProjectileAttack(tick, playerIndex, tower, ability, targetData, events);
        }

        /// <summary>
        /// 투사체 이동 및 충돌 처리를 업데이트합니다.
        /// </summary>
        public void TickProjectiles(long tick, int playerIndex, float deltaTime, List<MergeGameEvent> events)
        {
            if (events == null || _projectiles.Count == 0)
            {
                return;
            }

            _projectileRemoveBuffer.Clear();

            for (var i = 0; i < _projectiles.Count; i++)
            {
                var projectile = _projectiles[i];
                if (projectile.PlayerIndex != playerIndex)
                {
                    continue;
                }

                projectile.RemainingTime -= deltaTime;

                if (projectile.ProjectileType == ProjectileType.Throw && !projectile.IsLanded)
                {
                    if (projectile.RemainingTime > 0f)
                    {
                        continue;
                    }

                    // 착지 후 활성 상태가 됩니다. 히트가 발생할 때까지 유지합니다.
                    projectile.IsLanded = true;
                    projectile.RemainingTime = 0f;
                    continue;
                }

                if (projectile.RemainingTime > 0f)
                {
                    continue;
                }

                var shouldRemove = false;
                if (projectile.ProjectileType == ProjectileType.Throw)
                {
                    shouldRemove = ResolveThrowHit(tick, playerIndex, projectile, events);
                }
                else
                {
                    ResolveDirectHit(tick, playerIndex, projectile, events);
                    shouldRemove = true;
                }

                if (shouldRemove)
                {
                    _projectileRemoveBuffer.Add(projectile);
                }
            }

            for (var i = 0; i < _projectileRemoveBuffer.Count; i++)
            {
                _projectiles.Remove(_projectileRemoveBuffer[i]);
            }
        }

        private void ProcessHitScanAttack(
            long tick,
            int playerIndex,
            MergeTower tower,
            GameplayAbility ability,
            TargetData targetData,
            List<MergeGameEvent> events)
        {
            if (targetData == null)
            {
                return;
            }

            var sourceAsc = tower.ASC;
            var hitEffects = ability?.AppliedEffects;

            foreach (var targetAsc in targetData.Targets)
            {
                var monster = FindMonsterByAsc(playerIndex, targetAsc);
                if (monster == null || !monster.IsAlive)
                {
                    continue;
                }

                var actualDamage = ApplyHitEffects(sourceAsc, hitEffects, monster);

                events.Add(new TowerAttackedEvent(
                    tick,
                    playerIndex,
                    tower.Uid,
                    monster.Uid,
                    actualDamage,
                    tower.Position.X,
                    tower.Position.Y,
                    tower.Position.Z,
                    monster.Position.X,
                    monster.Position.Y,
                    monster.Position.Z,
                    TowerAttackType.HitScan,
                    tower.ProjectileType,
                    tower.ProjectileSpeed,
                    tower.ThrowRadius
                ));

                if (actualDamage > 0f)
                {
                    events.Add(new MonsterDamagedEvent(
                        tick,
                        playerIndex,
                        monster.Uid,
                        actualDamage,
                        monster.ASC.Get(AttributeId.Health),
                        tower.Uid
                    ));
                }
            }
        }

        private void ProcessProjectileAttack(
            long tick,
            int playerIndex,
            MergeTower tower,
            GameplayAbility ability,
            TargetData targetData,
            List<MergeGameEvent> events)
        {
            if (tower.ProjectileType == ProjectileType.Throw)
            {
                var throwPoint = ResolveThrowPoint(playerIndex, tower);

                var projUid = EnqueueProjectile(
                    tower,
                    playerIndex,
                    ability,
                    targetUid: 0,
                    impact: throwPoint,
                    projectileType: ProjectileType.Throw,
                    throwRadius: tower.ThrowRadius);

                events.Add(new TowerAttackedEvent(
                    tick,
                    playerIndex,
                    tower.Uid,
                    0,
                    0f,
                    tower.Position.X,
                    tower.Position.Y,
                    tower.Position.Z,
                    throwPoint.X,
                    throwPoint.Y,
                    throwPoint.Z,
                    TowerAttackType.Projectile,
                    ProjectileType.Throw,
                    tower.ProjectileSpeed,
                    tower.ThrowRadius,
                    projUid
                ));

                return;
            }

            if (targetData == null)
            {
                return;
            }

            foreach (var targetAsc in targetData.Targets)
            {
                var monster = FindMonsterByAsc(playerIndex, targetAsc);
                if (monster == null || !monster.IsAlive)
                {
                    continue;
                }

                var impact = monster.Position;

                var projUid = EnqueueProjectile(
                    tower,
                    playerIndex,
                    ability,
                    monster.Uid,
                    impact,
                    ProjectileType.Direct,
                    0f);

                events.Add(new TowerAttackedEvent(
                    tick,
                    playerIndex,
                    tower.Uid,
                    monster.Uid,
                    0f,
                    tower.Position.X,
                    tower.Position.Y,
                    tower.Position.Z,
                    impact.X,
                    impact.Y,
                    impact.Z,
                    TowerAttackType.Projectile,
                    ProjectileType.Direct,
                    tower.ProjectileSpeed,
                    tower.ThrowRadius,
                    projUid
                ));
            }
        }

        private long EnqueueProjectile(
            MergeTower tower,
            int playerIndex,
            GameplayAbility ability,
            long targetUid,
            Point3D impact,
            ProjectileType projectileType,
            float throwRadius)
        {
            var speed = tower.ProjectileSpeed > 0f ? tower.ProjectileSpeed : 1f;
            var distance = MathF.Sqrt(Point3D.DistanceSquared(tower.Position, impact));
            var travelTime = speed <= 0f ? 0f : distance / speed;
            var uid = _nextProjectileUid++;

            _projectiles.Add(new PendingProjectile
            {
                Uid = uid,
                PlayerIndex = playerIndex,
                AttackerUid = tower.Uid,
                TargetUid = targetUid,
                SourceAscSnapshot = CreateSourceSnapshot(tower.ASC),
                TargetingStrategy = ability?.TargetingStrategy,
                HitEffects = ability?.AppliedEffects != null ? new List<GameplayEffect>(ability.AppliedEffects) : null,
                Start = tower.Position,
                Impact = impact,
                RemainingTime = travelTime,
                TravelTime = travelTime,
                ProjectileType = projectileType,
                ThrowRadius = throwRadius,
                IsLanded = false
            });

            return uid;
        }
        /// <summary>
        /// ResolveDirectHit 메서드입니다.
        /// </summary>

        private void ResolveDirectHit(long tick, int playerIndex, PendingProjectile projectile, List<MergeGameEvent> events)
        {
            var monster = _state.GetMonster(playerIndex, projectile.TargetUid);
            if (monster == null || !monster.IsAlive)
            {
                return;
            }

            var actualDamage = ApplyHitEffects(
                projectile.SourceAscSnapshot,
                projectile.HitEffects,
                monster);

            if (actualDamage <= 0f)
            {
                return;
            }

            events.Add(new MonsterDamagedEvent(
                tick,
                playerIndex,
                monster.Uid,
                actualDamage,
                monster.ASC.Get(AttributeId.Health),
                projectile.AttackerUid
            ));
        }
        /// <summary>
        /// ResolveThrowHit 메서드입니다.
        /// </summary>

        private bool ResolveThrowHit(long tick, int playerIndex, PendingProjectile projectile, List<MergeGameEvent> events)
        {
            var targets = ResolveThrowTargets(playerIndex, projectile);
            if (targets.Count == 0)
            {
                return false;
            }

            var hasHit = false;
            for (var i = 0; i < targets.Count; i++)
            {
                var targetAsc = targets[i];
                var monster = FindMonsterByAsc(playerIndex, targetAsc);
                if (monster == null || !monster.IsAlive)
                {
                    continue;
                }

                var actualDamage = ApplyHitEffects(
                    projectile.SourceAscSnapshot,
                    projectile.HitEffects,
                    monster);

                if (actualDamage <= 0f)
                {
                    continue;
                }

                hasHit = true;
                events.Add(new MonsterDamagedEvent(
                    tick,
                    playerIndex,
                    monster.Uid,
                    actualDamage,
                    monster.ASC.Get(AttributeId.Health),
                    projectile.AttackerUid
                ));
            }

            return hasHit;
        }
        /// <summary>
        /// ResolveThrowTargets 메서드입니다.
        /// </summary>

        private IReadOnlyList<AbilitySystemComponent> ResolveThrowTargets(int playerIndex, PendingProjectile projectile)
        {
            if (projectile.SourceAscSnapshot != null && projectile.TargetingStrategy != null)
            {
                var impactContext = CreateImpactContext(playerIndex, projectile.SourceAscSnapshot, projectile.Impact);
                var targetData = projectile.TargetingStrategy.FindTargets(projectile.SourceAscSnapshot, impactContext);
                if (targetData != null && targetData.Targets.Count > 0)
                {
                    return targetData.Targets;
                }
            }

            _monsterAscList.Clear();
            var radiusSq = projectile.ThrowRadius * projectile.ThrowRadius;
            foreach (var monster in _state.GetAllMonsters(playerIndex))
            {
                if (!monster.IsAlive)
                {
                    continue;
                }

                var distSq = Point3D.DistanceSquared(projectile.Impact, monster.Position);
                if (distSq <= radiusSq)
                {
                    _monsterAscList.Add(monster.ASC);
                }
            }

            return _monsterAscList;
        }

        private float ApplyHitEffects(
            AbilitySystemComponent sourceAsc,
            IReadOnlyList<GameplayEffect> hitEffects,
            MergeMonster targetMonster)
        {
            var beforeHealth = targetMonster.ASC.Get(AttributeId.Health);

            if (sourceAsc == null || hitEffects == null || hitEffects.Count == 0)
            {
                return 0f;
            }

            var targetData = new TargetData(sourceAsc, targetMonster.Position);
            targetData.AddTarget(targetMonster.ASC);
            sourceAsc.ApplyEffectsToTargets(hitEffects, targetData);

            var afterHealth = targetMonster.ASC.Get(AttributeId.Health);
            var dealt = beforeHealth - afterHealth;
            return dealt > 0f ? dealt : 0f;
        }
        /// <summary>
        /// CreateSourceSnapshot 메서드입니다.
        /// </summary>

        private AbilitySystemComponent CreateSourceSnapshot(AbilitySystemComponent sourceAsc)
        {
            if (sourceAsc == null)
            {
                return null;
            }

            var snapshotAttributes = new AttributeSet();
            foreach (var value in sourceAsc.Attributes.Values)
            {
                snapshotAttributes.SetAttribute(
                    value.AttributeId,
                    value.CurrentValue,
                    value.MinValue,
                    value.MaxValue);
            }

            var snapshotTags = new GameplayTagContainer();
            var sourceTags = sourceAsc.OwnedTags.Tags;
            for (var i = 0; i < sourceTags.Count; i++)
            {
                snapshotTags.AddTag(sourceTags[i]);
            }

            return new AbilitySystemComponent(snapshotAttributes, new List<GameplayAbility>(), snapshotTags);
        }
        /// <summary>
        /// CreateImpactContext 메서드입니다.
        /// </summary>

        private TargetContext CreateImpactContext(int playerIndex, AbilitySystemComponent owner, Point3D impact)
        {
            return new TargetContext(
                getEnemies: _ => GetEnemiesForPlayer(playerIndex),
                getAllies: GetAlliesForTower,
                getPosition: asc => asc == owner ? impact : GetPositionForAsc(asc),
                random: _targetContext.Random);
        }
        /// <summary>
        /// ResolveThrowPoint 메서드입니다.
        /// </summary>

        private Point3D ResolveThrowPoint(int playerIndex, MergeTower tower)
        {
            var path = _state.GetMonsterPath(playerIndex, 0);
            if (path != null)
            {
                var progress = (float)_targetContext.Random.NextDouble();
                return path.GetPositionAtProgress(progress);
            }

            return tower.Position;
        }
        /// <summary>
        /// GetEnemiesForTower 메서드입니다.
        /// </summary>

        private IReadOnlyList<AbilitySystemComponent> GetEnemiesForTower(AbilitySystemComponent owner)
        {
            _monsterAscList.Clear();

            if (owner?.Owner is MergeTower tower)
            {
                for (var i = 0; i < _state.PlayerCount; i++)
                {
                    if (_state.GetTower(i, tower.Uid) != null)
                    {
                        return GetEnemiesForPlayer(i);
                    }
                }
            }

            return _monsterAscList;
        }
        /// <summary>
        /// GetEnemiesForPlayer 메서드입니다.
        /// </summary>

        private IReadOnlyList<AbilitySystemComponent> GetEnemiesForPlayer(int playerIndex)
        {
            _monsterAscList.Clear();
            foreach (var monster in _state.GetAllMonsters(playerIndex))
            {
                if (monster.IsAlive)
                {
                    _monsterAscList.Add(monster.ASC);
                }
            }

            return _monsterAscList;
        }
        /// <summary>
        /// GetAlliesForTower 메서드입니다.
        /// </summary>

        private IReadOnlyList<AbilitySystemComponent> GetAlliesForTower(AbilitySystemComponent owner)
        {
            return Array.Empty<AbilitySystemComponent>();
        }
        /// <summary>
        /// GetPositionForAsc 메서드입니다.
        /// </summary>

        private Point3D GetPositionForAsc(AbilitySystemComponent asc)
        {
            if (asc == null)
            {
                return Point3D.zero;
            }

            if (asc.Owner is MergeTower tower)
            {
                return tower.Position;
            }

            if (asc.Owner is MergeMonster monster)
            {
                return monster.Position;
            }

            return Point3D.zero;
        }
        /// <summary>
        /// FindMonsterByAsc 메서드입니다.
        /// </summary>

        private MergeMonster FindMonsterByAsc(int playerIndex, AbilitySystemComponent asc)
        {
            if (asc?.Owner is MergeMonster monster)
            {
                return monster;
            }

            return null;
        }
    }
}
