...
namespace MyProject.MergeGame.AI
{
    ...
    public sealed class MonsterPathMoveAI : IMergeMonsterAI
    {
            ...
            List<MergeHostEvent> events)
        {
            ...
            var pathCount = state.Paths.Count;
            if (pathCount <= 0)
            {
                return;
            }
            ...
            while (remainingDistance > 0f && guard < 64)
            {
                ...
                var path = state.GetMonsterPath(pathIndex);
                if (path == null || path.TotalLength <= 0f)
                {
                    pathIndex = GetNextValidPathIndex(state, pathIndex);
                    progress = 0f;
                    continue;
                }
            ...
            var finalPath = state.GetMonsterPath(pathIndex);
        ...
        private static int GetNextValidPathIndex(MergeHostState state, int currentPathIndex)
        {
            var count = state.Paths.Count;
            if (count <= 0)
            {
            ...
            {
                idx = (idx + 1) % count;
                var path = state.GetMonsterPath(idx);
                if (path != null && path.TotalLength > 0f)
                {
                    return idx;
                }
